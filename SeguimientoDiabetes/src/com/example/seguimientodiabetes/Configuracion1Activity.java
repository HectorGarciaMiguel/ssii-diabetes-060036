		...
		if ((keyCode == KeyEvent.KEYCODE_BACK)){
			...
			if (prefs.getString("visitado", "").equals("true")){
				...
				Toast mensaje =Toast.makeText(getApplicationContext(),
	                    "No hay configuración guardada. Continúe, son sólo 4 pasos", Toast.LENGTH_SHORT);
		        mensaje.show();
	...
	public void lanzarConfiguracion2(View v){
		...
		if (peso.getText().length()==0 || altura.getText().length()==0){
			...
			if ( hoy.get(Calendar.YEAR) <  anoNacimiento + 10 || hoy.get(Calendar.YEAR) >  anoNacimiento + 120){  
				Toast mensaje =Toast.makeText(getApplicationContext(),
	                    "Debe de introducir una fecha de nacimiento correcto, edad comprendida de 10 a 120 años.", Toast.LENGTH_LONG);
				mensaje.show();
			}else{
   ...
   protected Dialog onCreateDialog(int id) {
       ...
       case DATE_DIALOG_ID:
		...
		case ALERT_INFO:
           ...
           builder.setMessage("Estilo de vida según profesiones\n\n\n" +
           		"Actividad ligera: Oficinistas, médicos, enfermeras, jubilados...\n\n" +
           		"Actividad media: Abogados, amas de casa...\n\n" +
           		"Actividad intensa: Contrucción, industria ligera, pescadores...\n\n" +
